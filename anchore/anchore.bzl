...
def _grype_test_impl(ctx):
    if ctx.file.image and ctx.file.image.short_path:
        ...
        imglabel = ctx.file.image
        imgpath = _runfiles_path(ctx, imglabel)
    ...
    return [
        DefaultInfo(
            ...
            runfiles = ctx.runfiles(files = [ctx.executable._grype, imglabel]),
        )
    ]
...
def grype_test(name, image=None, sbom=None, fail_on_severity="low", scope="Squashed", **kwargs):
    ...
    # external Database.
    # TODO: Allow a configurable static database that will not use this
    # tag if specified.
    if 'tags' in kwargs:
